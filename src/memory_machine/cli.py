...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Memory-Machine thinks for you about the meanings.",
        epilog="Example:  memory-machine input_text.txt > output_text.txt"
...
def run():
    """
    $ text | memory-machine                        # Accepts text from the pipe
    $ echo "...<text>..." | memory-machine         #

    $ memory-machine multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="memory-machine",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"memory-machine: {utterance}.")
        syslog.closelog()
